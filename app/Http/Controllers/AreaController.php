    ...
    public function update(StoreAreaRequest $request, $area)
    {
        if (is_numeric($area)) {
            ...
            try {
                ...
                $oldArea = Area::findOrFail($area);

                $oldArea->id = $request->id;
                $oldArea->name = $request->name;
                $oldArea->address = $request->address;
                $oldArea->country_id = $request->country_id;

                $oldArea->save();
            } catch (\Illuminate\Database\QueryException $exception) {

                return to_route('areas.index')
                    ->with('error', 'Error in Updating Area!')
                    ->with('timeout', 3000);
            }
